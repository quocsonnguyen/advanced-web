...
function Authorize(props) {
    ...
    const _userID = { id: qs.parse(window.location.search, { ignoreQueryPrefix: true })._id }

    useEffect(() => {
        async function getUser() {
            const response = await axios.get(`/api/user/${_userID.id}`)
    ...
    return (
        <Layout>
            <Container>
                ...
                <Row>
                    <button onClick={handleSave} className={style.authorize_button}> Cập Nhật </button>
                </Row>
            </Container>
        </Layout>
    )
